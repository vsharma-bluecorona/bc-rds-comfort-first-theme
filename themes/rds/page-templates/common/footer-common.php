...
<?php } elseif ($heroItems['type'] == 'service_titan') {
    ...
    <?php
} elseif ($desktopItems['type'] === 'zocdoc') {
    ?>
    <script type="text/javascript">
    jQuery(document).ready(function(){
        var zocdocContent = '<?php echo $get_rds_tracking['scheduler']['zocdoc_content']; ?>';
...
//CODE FOR OVERLAPING TEXT FIELDS AFTER SUBMITION END
    ...
    jQuery(document).ready(function () {
        ...
        jQuery(".apply-conditional-color").each(function (index) {
            let color_bg_elem = jQuery(this).parents('.container-fluid, #cta-a ');
            

// let model_color_bg = jQuery(this).parents('#cta-a').css("background-color");
            if(color_bg_elem) {
                // Added this to ensure there is no null element - 01/April/2024 - Girish
                let color_bg = color_bg_elem.css("background-color");
                // Added this to ensure there is no null element - 01/April/2024 - Girish
                if(color_bg) {
...
<?php if ($get_rds_tracking['tracking']['enable'] == true) { ?>
